...
genetic.mutate = function(entity) {
  ...
  while (
    indexToDecrement == indexToIncrement ||
    entity[indexToDecrement] <= 4
  ) {
...
genetic.fitness = function(entity) {
  ...
  for (let i in global["defenders"]) {
    ...
    for (let j in global["defenders"][i].moves) {
      const move = new global["Move"](
        8,
        global["defenders"][i].moves[j]["move"]
      );
      if (
        move.bp > 0 &&
        [].indexOf(move.name) === -1 &&
        ...
        const calc = new global["Calc"](
          global["defenders"][i].name,
          global["poke"],
          { move: move.name },
          spread
        );
    ...
    for (let j in global["attackerUsage"].moves) {
      const move = new global["Move"](
        8,
        global["attackerUsage"].moves[j]["move"]
      );
      if (
        move.bp > 0 &&
        [].indexOf(move.name) === -1 &&
